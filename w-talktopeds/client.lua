    ...
    while true do
        for k, v in pairs(Config.Peds) do
            ...
            if not spawned then
                ...
                if dist <= 200 then
                    TriggerEvent("w-talktopeds:client:SpawnPed", k)
                    break
                end
            end
        end
...
RegisterNetEvent('w-talktopeds:client:SpawnPed', function(SpawnedPed)
...
RegisterNetEvent('w-talktopeds:client:NPC')
AddEventHandler('w-talktopeds:client:NPC', function(data, hook, pline, oline)
    local myMenu
    local Name
    local msg
    local ped
    ...
    if line ~= 0 then
        ...
        if Config.Menu == "qb" then
            ...
            myMenu[#myMenu+1] = {
                    header = msg,
                    params = {
                    event = "w-talktopeds:client:Talk",
                    args = {
                        ped = ped,
                        hookto = line
                    }
...
RegisterNetEvent('w-talktopeds:client:Talk')
AddEventHandler('w-talktopeds:client:Talk', function(data, hook)
    local myMenu
    local line = 1
    local msg
    local ped
